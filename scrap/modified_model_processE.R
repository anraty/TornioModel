...
#   removed, not needed for tornio only
...
for(s in stock:stock){              #s index denotes AU here 1 for tornio
    for (i in 1:m){ 
...
for(i in 1:(m+proj_years)){
    ...
    surv[i,1,6,1]<-(1-HtW[i,1]) * exp(-MpsW[i]/Tstep) 	         
...
for(i in 1:(m+proj_years)){
    for(j in 1:6){           
        for(ii in 1:4){
            ...
            #   might need a change only for wild
        ...
        minvr[i,j]<-min(vvErw[i,j,stock:stock],vvErr[i,j])      #compare W with R
        ...
        #   these sums need to be modified                                                                 
        # minvc[i,j]<-min(vvEc[i,j,1:2,1:AUS])      #compare W with R
        # minvdc[i,j]<-min(vvEdc[i,j,1:2,1:AUS])      #compare W with R
        minvc[i,j]<-min(vvEc[i,j,1:2,])      #compare W with R
        minvdc[i,j]<-min(vvEdc[i,j,1:2,])      #compare W with R
